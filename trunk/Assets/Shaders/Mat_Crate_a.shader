...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_node_3,TRANSFORM_TEX(node_37.rg, _node_3))).rgb;
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = tex2D(_node_4,TRANSFORM_TEX(node_37.rg, _node_4)).rgb;
                float3 specular = (floor(attenuation) * _LightColor0.xyz) * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * tex2D(_node_2,TRANSFORM_TEX(node_37.rg, _node_2)).rgb;
                finalColor += specular;
/// Final Color:
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_node_3,TRANSFORM_TEX(node_38.rg, _node_3))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = tex2D(_node_4,TRANSFORM_TEX(node_38.rg, _node_4)).rgb;
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * tex2D(_node_2,TRANSFORM_TEX(node_38.rg, _node_2)).rgb;
                finalColor += specular;
/// Final Color:
